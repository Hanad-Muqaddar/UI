...
function action_trigger() {
    ...
    $.ajax({
        // url: `http://35.208.146.128/conversations/${user_id}/execute`,
...
function send(message) {
    ...
    $.ajax({
        // url: "http://35.208.146.128:80/webhooks/rest/webhook",
            ...
            for (i = 0; i < response.length; i++) {
                ...
                if (response[i].hasOwnProperty("attachment")) {
                    ...
                    if (response[i].attachment.type == "video") {
                        ...
                        picker = new Lightpick({ field: document.getElementById('userInput') });
                        // '+' <button id="datepicker">Please Enter Date</button> '+'
                        // var BotResponse = '<div class="video-container"> <iframe src="' + video_url + '" frameborder="0" allowfullscreen></iframe> </div>'
                        var BotResponse ='<div></div>'
